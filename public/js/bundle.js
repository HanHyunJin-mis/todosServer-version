...
var $clearbtn = document.querySelector('.btn');
var $completeTodos = document.querySelector('.completed-todos');
var $activeTodos = document.querySelector('.active-todos'); // 리스트 추가하기

var todos = [];
var navId = 'all'; // completed clear

function clearcompleted() {
  $completeTodos.innerHTML = todos.filter(function (item) {
    return item.completed;
  }).length;
  $activeTodos.innerHTML = todos.filter(function (item) {
    return !item.completed;
  }).length;
}
...
function render(todosFromServer) {
  ...
  var _todos = todos.filter(function (item) {
    if (navId === 'active') return !item.completed;
    if (navId === 'completed') return item.completed;
    return true;
  });

  var html = ''; // eslint-disable-next-line no-return-assign
...
$todos.onchange = function (e) {
  ...
  patchOneTodo(+e.target.parentNode.id, {
...
$nav.onclick = function (e) {
  _toConsumableArray($nav.children).forEach(function (item) {
